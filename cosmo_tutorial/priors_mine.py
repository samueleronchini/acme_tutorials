...
def make_pos_def(cov, eps=1e-8):
    """
    Force covariance matrix to be positive semidefinite.
    Replaces any eigenvalue below eps with eps and logs the change.
    """
    vals, vecs = np.linalg.eigh(cov)
    vals_clipped = np.clip(vals, eps, None)  # clip all small/negative eigenvalues
    cov_fixed = vecs @ np.diag(vals_clipped) @ vecs.T

    # --- diagnostics: measure how much the matrix changed ---
    frob_diff = np.linalg.norm(cov_fixed - cov, ord='fro')
    frob_rel = frob_diff / np.linalg.norm(cov, ord='fro')
    eig_diff = np.max(np.abs(vals_clipped - vals))
    eig_rel = eig_diff / (np.max(np.abs(vals)) + eps)

    logging.info(f"Covariance regularized: Frobenius relative change={frob_rel:.2e}, "
                 f"Max relative eigenvalue change={eig_rel:.2e}")

    return cov_fixed

def truncated_check(values, c_matrix, minima, maxima, samp):
    """
    Sample from a TruncatedMVN, correcting the covariance matrix
    if it is not positive semidefinite.
    """
    running_error = False
    try:
        tmvn = TruncatedMVN(values, c_matrix, minima, maxima)
        smpl = tmvn.sample(samp)
    except Exception:
        running_error = True
        logging.warning("Covariance not PSD, applying eigenvalue clipping...")
        c_matrix = make_pos_def(c_matrix)
        tmvn = TruncatedMVN(values, c_matrix, minima, maxima)
        smpl = tmvn.sample(samp)
    return smpl, running_error
...
def uniform_in_differential_comoving_volume_Class(cosmology, x, a, b, z):

    c_km_s = 299792.458
    H0=cosmology.Hubble(0) * c_km_s

    #dVc_dz_dOm = lambda z: np.array([cosmology.comoving_distance(zi)**2.0 / cosmology.Hubble(zi) for zi in z])
    dVc_dz_dOm = lambda z: np.array([((1 + zi) * cosmology.angular_distance(zi))**2.0 / cosmology.Hubble(zi) for zi in z])

    H = lambda z: np.array([cosmology.Hubble(zi)* c_km_s for zi in z])
    
    return np.where(np.logical_and(z >= a, z <= b),  dVc_dz_dOm(z)/(1+z) * (x/(1+z)+ c_km_s * (1+z) / H(z))**(-1), 0)
...
def get_truncated_likelihood_samples(params, mean_values, cov_matrix, num_samples, min_array = None, max_array = None):
    ...
    i = 0
    #list_flipping = [1e-10, 5e-10, 1e-9, 5e-9, 8e-9, 1e-8, 3e-8, 5e-8, 8e-8, 1e-7, 3e-7, 5e-7, 8e-7, 1e-6, 3e-6, 5e-6, 8e-6, 1e-5, 3e-5, 5e-5, 8e-5, 1e-4, 3e-4, 5e-4, 8e-4, 1e-3, 3e-3, 5e-3, 8e-3, 1e-2, 3e-2, 5e-2, 8e-2, 1e-1, 3e-1, 5e-1, 8e-1]
    samples_tmvn = np.zeros((len(mean_values), num_samples))
    zeros_tmvn = np.zeros((len(mean_values), num_samples))

    while np.sum(samples_tmvn) == 0:

        samples_tmvn, running_error=truncated_check(mean_values, np.copy(cov_matrix), min_array, max_array, num_samples)
        i+=1

    # if running_error:

    #     logging.warning(f'Flipping value is {list_flipping[i-1]}')

    return pd.DataFrame(samples_tmvn.T, columns=params)

def get_posteriors_samples(params, likelihood_samples, num_posterior_samples, priors_dict = None, cosmology = None, dL_z = None):
    """
    ...
    for var in params:
        ...
        if var != 'luminosity_distance':
            ...
            if priors_dict[var]['prior_type'] != 'uniform_in_differential_comoving_volume_Class':
                prior *= globals()[priors_dict[var]['prior_type']](likelihood_samples[var].to_numpy(), priors_dict[var]['lower_prior_bound'], priors_dict[var]['upper_prior_bound'])
            else:
                prior *= globals()[priors_dict[var]['prior_type']](cosmology, likelihood_samples[var].to_numpy(), priors_dict[var]['lower_prior_bound'], priors_dict[var]['upper_prior_bound'], dL_z)
