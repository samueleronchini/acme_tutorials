...
class TruncatedMVN:
    ...
    def sample(self, n):
        ...
        while accept < n:
            ...
            iteration += 1
            if iteration == 10 ** 3:
                warnings.warn('Warning: Acceptance prob. smaller than 0.001.', UserWarning)
            elif iteration > 10 ** 4:
                accept = n
                rv = np.concatenate((rv, Z), axis=1)
                warnings.warn('Warning: Sample is only approximately distributed.', UserWarning)
    ...
    def compute_factors(self):
        ...
        if np.any(D < self.eps):
            warnings.warn('Warning: Method might fail as covariance matrix is singular!', UserWarning)
...
if __name__ == '__main__':
    ...
    ax1.plot(x_test, stats.norm.pdf(x_test, mu_test[idx_test], np.sqrt(cov_test[idx_test, idx_test])),
